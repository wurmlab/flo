...
each transcript, you will need to remove that:
    ...
    /path/to/flo/gff_remove_feats.rb gene xx_genes.gff > xx_genes_cleaned.gff

Finally, run flo as:

    rake -f /path/to/flo/Rakefile

This will generate a directory called 'run/'. A directory is created for each
given GFF3 file in 'run/' that contains:
